...
// Static HR/system messages always shown
...
export default function Inbox() {
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      // Convert backend notifications to message format
      const notifMsgs: Message[] = data.map(n => ({
        id:       `n_${n.id}`,
        notifId:  n.id,
        sender:   'Admin — Project Allocation',
        avatar:   n.role === 'Project Head' ? 'PH' : 'TM',
        color:    n.role === 'Project Head' ? '#e63946' : '#4cc9f0',
        subject:  `Project Allocated: ${n.projectName}`,
        body:     n.message || `You have been allocated to project "${n.projectName}".`,
        time:     formatRelativeTime(n.sentAt || n.createdAt),
        date:     new Date(n.sentAt || n.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }),
        read:     n.read || false,
        starred:  false,
        category: 'Allocation',
      }))

      // Merge: notifications first, then static
      setMsgs([...notifMsgs, ...STATIC_MESSAGES])
    } catch {
      // Keep static messages only
  ...
  const markRead = async (msg: Message) => {
    ...
    if (msg.notifId) {
      try { await fetch(`${API}/api/notifications/${msg.notifId}/read`, { method: 'PATCH' }) }
      catch {}
    }
  }
